...
class Cart{
    ...
    public function get_cart_items(){
        $stmt = $this->conn->prepare("SELECT p.product_id , p.name, p.price , p.size , p.image , c.quantity
                                    ...
                                    WHERE c.user_id = ?");
    $stmt->bind_param("i",$_SESSION['user_id']);//tuka ova i ni e za prasalnikot kaj c.user_id =? bidejki toa ni e integer vrednost 
    //i zimame go od sesijata onoj koj sto e momentalno najaven
    $stmt->execute();
