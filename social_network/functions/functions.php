...
function email_exists($email){//proveruvame dali mailot e zafaten
...
function user_exists($username){//proveruvame dali username-ot e zafaten
...
function validate_user_registration(){
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(strlen($first_name)<3){
            $errors[]= "Your First Name cannot be less then 3 charachers!";
        }

        if(strlen($last_name)<3){
            $errors[]= "Your Last Name cannot be less then 3 charachers!";
        }

        if(strlen($username)<3){
            $errors[]= "Your UserName cannot be less then 3 charachers!";
        }

        if(strlen($username)>20){
            $errors[]= "Your UserName cannot be bigger then 20 charachers!";
        }

        if(email_exists($email)){
            $errors[]= "Your Email is already is taken!";
        }

        if(user_exists($username)){
            $errors[]= "Your Username is already is taken!";
        }

        if(strlen($password)<8){
            $errors[]= "Your Password cannot be less then 8 charachers!";
        }
...
function create_user($first_name,$last_name,$username,$email,$password){
        ...
        VALUES('$first_name','$last_name','$username','uploads/default.jpg','$email','$password_has')";
...
function user_profile_image_upload(){
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($imageFileType !="jpg" && $imageFileType !="png" && $imageFileType !="jpeg" && $imageFileType !="gif"){
            ...
            $error="Sorry, your only JPG, JPGE, PNG &GIF files are allowed.";
            $uploadOk=0;
        }
...
function create_post(){
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(strlen($post_content)<1){
            $errors="Your post content must be large!";
        }
        if(!empty($errors)){
            foreach($errors as $error){
               echo "<div class='alert'>" . $error . "</div>";
